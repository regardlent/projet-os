...
- **`model project`** (nouveau, **autonome**) : le CLI **écrit un projet complet** (impl + `main.cpp` + `CMakeLists.txt`, isolé sous `work/<name>/`), le **compile** et le **lance** — modèle choisi par le routeur, génération bornée (anti-blocage). **Multi-fichiers** via `--impl=a,b,...` (le `main` inclut toutes les unités et appelle `run()`).
