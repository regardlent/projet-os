...
static int cmdHelp(bool colorOn) {
	...
	auto pick = [&](const char* fr, const char* en) { return pos::g_lang == pos::Lang::En ? en : fr; };
	std::cout << green << "Project OS CLI v3 — usage (categorized)" << reset << "\n";
	sec(pick("Général", "General"));
	...
	sec(pick("Intelligence & analyse", "Intelligence & analysis"));
	...
	sec(pick("Artefact & config", "Artifact & config"));
	...
	std::cout << "  model show|smoke|benchmark|qualify|compare|flash|policy|quota|profiles|offload|cache|write <id>\n";
	...
	sec(pick("Qualité & release", "Quality & release"));
...
// F89 model write <relpath> <objective...>: generate content via LocalAI and write it into the active project workspace.
